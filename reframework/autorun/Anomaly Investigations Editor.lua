...
local function get_mystery_quest_data_table()
    ...
    for i,quest in ipairs(mystery_quest_data) do
        ...
        if quest_no ~= -1 then
            ...
            local monster0 = quest_monsters:get_element(0):get_field('value__')
            ...
            mystery_quests[ key ] = {
                                    ...
                                    monster0=monster0,
                                    monster1=quest_monsters:get_element(1):get_field('value__'),
                                    monster2=quest_monsters:get_element(2):get_field('value__'),
                                    monster5=quest_monsters:get_element(5):get_field('value__'),
...
local function generate_random()
	...
	em_types:call('set_Item',0,1)
	em_types:call('set_Item',1,1)
	em_types:call('set_Item',2,1)
...
re.on_draw_ui(function()
	if imgui.tree_node('Anomaly Investigations Editor') then
		...
		if qc_open or game_state ~= 4 or get_questman():call('isActiveQuest') then
	        ...
	        if quest then
				...
				imgui.same_line()
				if imgui.button('Generate Random Quest') then generate_random() end
